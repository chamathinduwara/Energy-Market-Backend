...
//
//        try {
//            SignedTransaction result = proxy.startTrackedFlowDynamic(ProposalFlow.Initiator.class, isBuyer, amount, unitPrice, otherParty).getReturnValue().get();
...
//
//        try{
//            SignedTransaction result = proxy.startTrackedFlowDynamic(ModificationFlow.Initiator.class, proposalId, newAmount, newUnitPrice).getReturnValue().get();
//            return  ResponseEntity
//                    .status(HttpStatus.CREATED)
//                    .body("Transaction id"+ result.getId() + "committed to ledger. \n" + result.getTx().getOutput(0));
//
