    ...
    public void proposalAcceptanceTransactionsHaveOnlyOneInputAndOneOutputState(){
        ledger(ledgerServices, ledger ->{
            ledger.transaction(tx ->{
                ...
                tx.output(ProposalAndTradeContract.ID, new TradeState(1.0,1.0,0.5,alice.getParty(), bob.getParty()));
                tx.verifies();
                return null;
            });
            return null;
    ...
    public void inputProposalStateAndOutputTradeStateShouldHaveExactlyTheSameAmount(){
        ledger(ledgerServices, ledger ->{
            ledger.transaction(tx ->{
                ...
                tx.tweak(tdsl ->{
                    tdsl.input(ProposalAndTradeContract.ID,new ProposalState(1.0,1.0,0.5, alice.getParty(), bob.getParty(), alice.getParty(), bob.getParty()));
                    tdsl.output(ProposalAndTradeContract.ID, new TradeState(2.0, 1.0,0.5,alice.getParty(), bob.getParty()));
                    tdsl.fails();
                    return null;
                });
                tx.tweak(tdsl ->{
                    tdsl.input(ProposalAndTradeContract.ID, new ProposalState(2.0,1.0,0.5, alice.getParty(), bob.getParty(), alice.getParty(), bob.getParty()));
                    tdsl.output(ProposalAndTradeContract.ID, new TradeState(1.0,1.0,0.5, alice.getParty(), bob.getParty()));
                    tdsl.fails();
                    return null;
                });
    ...
    public void buyerAndSellerAreUnmodifiedInTheOutput(){
        ledger(ledgerServices, ledger ->{
            ledger.transaction(tx ->{
                ...
                tx.command(ImmutableList.of(alice.getPublicKey(), bob.getPublicKey()), new ProposalAndTradeContract.Commands.Accept());
                tx.tweak(tdsl ->{
                    tdsl.output(ProposalAndTradeContract.ID, new TradeState(1.0, 1.0,0.5,alice.getParty(),  charlie.getParty()));
                    tdsl.fails();
                    return null;
                });
                tx.tweak(tdsl ->{
