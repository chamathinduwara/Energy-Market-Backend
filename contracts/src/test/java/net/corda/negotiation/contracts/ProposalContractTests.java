        ...
        ledger(ledgerServices, ledger ->{
            ledger.transaction(tx ->{
                ...
                tx.tweak(tdsl ->{
                    ...
                    tdsl.output(ProposalAndTradeContract.ID, new ProposalState(2.0,1.0,0.5, alice.getParty(), bob.getParty(), bob.getParty(), alice.getParty()));
                    tdsl.verifies();
                    return null;
                });
                tx.tweak(tdsl -> {
                ...
                tx.tweak(tdsl -> {
                    tdsl.output(ProposalAndTradeContract.ID, new ProposalState(2.0,1.0,0.5, alice.getParty(), charlie.getParty(), alice.getParty(), bob.getParty()));
                    tdsl.fails();
                    return null;
                });
                tx.output(ProposalAndTradeContract.ID, new ProposalState(2.0,1.0,0.5, alice.getParty(), bob.getParty(), alice.getParty(), bob.getParty()));
